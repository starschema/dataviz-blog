...
export default function TableauOverlay(props: Props) {
    ...
    return <>
        <div className='absolute top-0 h-full w-full cursor-zoom-in' onClick={() => setIsModalOpen(!isModalOpen)}>

        </div>
        <Modal open={isModalOpen} onClose={() => setIsModalOpen(!isModalOpen)}>
            <Image src={imageUrl} alt={props.alt} width={props.imageWidth} height={props.imageHeight} />
        </Modal>
    </>
    return
}